...
namespace ai{
    ...
    /// \brief Convert number co counter-like string
    ...
    /// \param symbolsBeforePoint Minimal required number of symbols before point
    ...
    INLINE std::string prependNumber(
        ...
        const int counter = symbolsBeforePoint - (ai::string((int) value)).size();
    ...
    INLINE bool loadA3R(
        ...
        std::ifstream a3r(filename, std::ios::binary | std::ios::in);
        ...
        a3r.seekg(4);
        a3r.read(numberOfParticlesValue, 4);
        a3r.read(startByte, 4);
        a3r.seekg(24);
        a3r.read(radiusValue, 8);
        a3r.seekg(reinterpret_cast<int&>(startByte));
    ...
    INLINE bool saveXYZ(
        ...
        std::ofstream xyz(filename + std::string(".xyz"));
        ...
        xyz << numberOfParticles << std::endl << std::endl;

        if(2 == positions[0].size()){
            for(std::size_t i = 0; i < numberOfParticles; ++i){
                xyz << "C " << positions[i][0] << " " << positions[i][1]
                    << " 0" << std::endl;
            }
        }else{
            if(3 == positions[0].size()){
    ...
    //#if defined(DIRENT_SUPPORT)
        /// \todo Add description. Add tests
        INLINE std::vector<std::string> listFilesWithExtensionAndPath(
            ...
            if(NULL != dir){
                while(NULL != (ent = readdir (dir))){
                    if(
                        ...
                        files.push_back(path+"/"+prefix + ent->d_name);
                    }
                }
